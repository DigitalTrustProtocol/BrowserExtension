...
export async function removeAllowedDomain(domain: string): Promise<boolean> {
    ...
    // Consuming the one-time X auto-connect means a later visit will not
    // silently reconnect after the user removes x.com / twitter.com.
    if (isXProductHost(domain)) {
        await markXHostAutoConnectDone();
    }
    return true;
}

export async function isXHostAutoConnectDone(): Promise<boolean> {
    const data = await browser.storage.local.get(X_HOST_AUTO_CONNECT_DONE_KEY);
    return Boolean((data as Record<string, unknown>)[X_HOST_AUTO_CONNECT_DONE_KEY]);
}

export async function markXHostAutoConnectDone(): Promise<void> {
    await browser.storage.local.set({ [X_HOST_AUTO_CONNECT_DONE_KEY]: true });
}

/**
 * First visit to x.com / twitter.com: allowlist once. After that (including
 * after the user disconnects), require an explicit Connect action.
 */
export async function maybeOneTimeAutoConnectXHost(domain: string): Promise<boolean> {
    const allowed = await getAllowedDomains();
    const done = await isXHostAutoConnectDone();
    if (!shouldOneTimeAutoConnectXHost(domain, allowed, done)) {
        // Already connected before the flag was written (legacy / manual) —
        // consume the offer so a later disconnect stays disconnected.
        if (isXProductHost(domain) && !done && allowed.includes(domain)) {
            await markXHostAutoConnectDone();
        }
        return false;
    }
...
export const handlers = new Map<string, HandlerFn>([
    ...
    ['maybeAutoConnectXHost', async (params) =>
        maybeOneTimeAutoConnectXHost(params.domain as string)],
